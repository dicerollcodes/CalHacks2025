  ...
  useFrame(() => {
    ...
    if (showParticles && particlesRef.current) {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        // Slow down
        particles.velocities[i] *= 0.96
        particles.velocities[i + 1] *= 0.96
        particles.velocities[i + 2] *= 0.96
      }
...
function RotatingCube({ onShatter, shattering }) {
  ...
  useFrame((state, delta) => {
    ...
    if (meshRef.current && shattering) {
      ...
      // Phase 1: Spin acceleration (0-0.8s)
      if (shatterTimer.current < 0.8) {
        const acceleration = shatterTimer.current / 0.8
        setSpinSpeed(0.25 + acceleration * 8)
        meshRef.current.rotation.x += delta * spinSpeed
        meshRef.current.rotation.y += delta * spinSpeed * 1.2
      }
      // Phase 2: Explosion (0.8s+)
      else {
        setScale(prev => prev * 0.90)
...
export default function IceCube({ onShatter, disabled }) {
  ...
  const handleShatter = () => {
    ...
    setShattering(true)
    if (onShatter) {
      // Wait for spin acceleration (800ms) + explosion start
      setTimeout(() => onShatter(), 1000)
    }
  }

  return (
    <div className="w-full h-64 cursor-pointer flex items-center justify-center">
      <Canvas
      ...
      >
        ...
        <directionalLight position={[3, 4, 3]} intensity={6} color="#ffffff" />

        {/* Fill light - soft blue from side */}
        <directionalLight position={[-4, 2, 3]} intensity={5} color="#7dd3fc" />

        {/* Rim light - highlight edges from behind */}
        <directionalLight position={[0, -2, -4]} intensity={4} color="#38bdf8" />

        {/* Accent lights for glow */}
        <pointLight position={[2, 2, 4]} intensity={5} color="#e0f2fe" />
        <pointLight position={[-2, -2, 4]} intensity={4} color="#7dd3fc" />
        <pointLight position={[0, 0, 3]} intensity={6} color="#ffffff" />
        ...
        <directionalLight position={[0, 0, -5]} intensity={4} color="#7dd3fc" />
